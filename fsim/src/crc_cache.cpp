...
//    out<<endl;
    ...
    out << " Misses : " << totMisses << endl;   
    cacheReplState->PrintStats( out, totMisses );
...
bool CRC_CACHE::LookupAndFillCache( UINT32 tid, Addr_t PC, Addr_t paddr, UINT32 accessType ) 
{
    ...
    if( wayID == -1 ) 
    {
        ...
        if( wayID != -1 )
        {
            ...
            cacheReplState->UpdateReplacementState( setIndex, wayID, currLine, tid, PC, accessType, hit , paddr);
        }
        else
            assert(0);
